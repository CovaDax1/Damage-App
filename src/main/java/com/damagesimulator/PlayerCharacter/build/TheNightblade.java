...
import com.damagesimulator.global.d8;
...
public class TheNightblade<M extends MeleeWeapon> extends PlayerCharacter<M> implements Hexblade, SpellCaster, PactCaster {
    ...
    public int pactSmite() {
        if (getPactCasterLevel() < 1 || warlockLevels < 3 || getPactSlots().getAvailablePactSlots() < 1) return 0;
        getPactSlots().spendPactSlot();
        return d8.getDie().roll(this.pactSlots.getPactSlotLevel() + 1);
    }
